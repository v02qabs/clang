...
HBITMAP hImage = NULL;
ULONG_PTR gdiToken;
char currentImagePath[MAX_PATH] = "";

// --- Save Diary Entry (Text + Image) ---
void SaveEntry(HWND hWnd)
{
    SYSTEMTIME st;
    DateTime_GetSystemtime(GetDlgItem(hWnd, IDC_DATE), &st);
    char fname[MAX_PATH], imgname[MAX_PATH];
    sprintf(fname, "%04d%02d%02d.txt", st.wYear, st.wMonth, st.wDay);
    sprintf(imgname, "%04d%02d%02d.jpg", st.wYear, st.wMonth, st.wDay);
    ...
    // --- Save image (copy original file) ---
    if (strlen(currentImagePath) > 0 && PathFileExistsA(currentImagePath))
    {
        CopyFileA(currentImagePath, imgname, FALSE);
    }

    MessageBox(hWnd, "Diary entry saved (text + image)!", "Info", MB_OK | MB_ICONINFORMATION);
}
...
void LoadEntry(HWND hWnd)
{
    ...
    {
        SetWindowText(GetDlgItem(hWnd, IDC_EDIT), "");
        MessageBox(hWnd, "No diary text found for this date.", "Warning", MB_OK | MB_ICONWARNING);
    }

    // --- Load image ---
    if (PathFileExistsA(imgname))
    {
        WCHAR wfile[MAX_PATH];
        MultiByteToWideChar(CP_ACP, 0, imgname, -1, wfile, MAX_PATH);
    ...
    {
        SendMessage(GetDlgItem(hWnd, IDC_IMAGE), STM_SETIMAGE, IMAGE_BITMAP, NULL);
